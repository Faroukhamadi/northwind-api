...
// The Country_languageFunc type is an adapter to allow the use of ordinary
// function as Country_language mutator.
type Country_languageFunc func(context.Context, *ent.CountryLanguageMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f Country_languageFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.CountryLanguageMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CountryLanguageMutation", m)
	}
	return f(ctx, mv)
}
