...
func (cq *CityQuery) sqlAll(ctx context.Context) ([]*City, error) {
	...
	if query := cq.withCountry; query != nil {
		...
		nodeids := make(map[string][]*City)
		for i := range nodes {
			if nodes[i].country_capital == nil {
				continue
			}
			fk := *nodes[i].country_capital
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "country_capital" returned %v`, n.ID)
			}
			for i := range nodes {
