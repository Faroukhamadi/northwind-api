...
func (cc *CityCreate) SetCountryID(id string) *CityCreate {
...
func (cc *CityCreate) check() error {
	...
	if _, ok := cc.mutation.District(); !ok {
		return &ValidationError{Name: "district", err: errors.New(`ent: missing required field "City.district"`)}
	}
	if _, ok := cc.mutation.Population(); !ok {
		return &ValidationError{Name: "population", err: errors.New(`ent: missing required field "City.population"`)}
	}
