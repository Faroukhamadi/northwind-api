...
type CountryQuery struct {
	...
	// eager-loading edges.
	withCapital  *CityQuery
	withLanguage *CountryLanguageQuery
...
// QueryCapital chains the current query on the "capital" edge.
func (cq *CountryQuery) QueryCapital() *CityQuery {
...
func (cq *CountryQuery) QueryLanguage() *CountryLanguageQuery {
	query := &CountryLanguageQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CountryQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (cq *CountryQuery) FirstIDX(ctx context.Context) string {
...
func (cq *CountryQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (cq *CountryQuery) OnlyIDX(ctx context.Context) string {
...
func (cq *CountryQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (cq *CountryQuery) IDsX(ctx context.Context) []string {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CountryQuery) Clone() *CountryQuery {
	...
	return &CountryQuery{
		...
		withCapital:  cq.withCapital.Clone(),
		withLanguage: cq.withLanguage.Clone(),
...
// WithCapital tells the query-builder to eager-load the nodes that are connected to
// the "capital" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CountryQuery) WithCapital(opts ...func(*CityQuery)) *CountryQuery {
	query := &CityQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCapital = query
	return cq
}
...
func (cq *CountryQuery) WithLanguage(opts ...func(*CountryLanguageQuery)) *CountryQuery {
	query := &CountryLanguageQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withLanguage = query
	return cq
}
...
func (cq *CountryQuery) sqlAll(ctx context.Context) ([]*Country, error) {
	...
	if query := cq.withCapital; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*Country)
		for i := range nodes {
		...
		query.Where(predicate.City(func(s *sql.Selector) {
			s.Where(sql.InValues(country.CapitalColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.country_capital
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "country_capital" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "country_capital" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Capital = n
		}
	}

	if query := cq.withLanguage; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*Country)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.Country_language(func(s *sql.Selector) {
			s.Where(sql.InValues(country.LanguageColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.country_language
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "country_language" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "country_language" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Language = n
		}
	}
